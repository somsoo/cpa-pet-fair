...
def create_text_thumbnail(text, filename_prefix):
    try:
        ...
        if not lines: lines = ["2026 케이펫페어 송도 (사전등록 무료관람)", "핵심 실무 가이드"]
...
def generate_post(campaign, keyword):
    ...
    draft_prompt = f"""당신은 반려동물 영양관리사이자 펫페어 트렌드 전문 에디터입니다.
...
---METADATA_END---"""
    ...
    vibe_keywords = meta.get('vibe_keywords', 'dog cat puppy')
